         ...
         <div class="panel hidden-xs">
            ...
            <div class="panel-body">
               <form id="form">
                  ...
                  <input type="hidden" name="suc_id" value="<?php echo $get_branch->suc_id;?>">
                  ...
                  <p>Seleccione en que días y rangos horarios desea otorgar turnos para este prestador. En caso de trabajar de corrido solo usar el horario 1. Ej. Horario corrido: 9 a 18 hs. / Horario cortado: 9 a 14 y 17 a 20 hs. Esta configuración tomara efecto en todos aquellos prestadores que no tengan configurado sus propios días y horarios, y que su Sucursal correspondiente tampoco se encuentre configurado.</p>
                  <div class="form-group btop " >
                     <label class="label-1">Horarios de atención:</label>
                  </div>
                  <?php  $count_shedules_lenders=DB::table('tu_dlab')->where('lab_presid',$get_lender->tmsp_id)->count();?>

                  <?php  $count_shedules_branch=DB::table('tu_dlab')->where('lab_empid',$get_business->em_id)->where('lab_sucid',$get_branch->suc_id)->where('lab_presid',0)->count();;?>

                  
                  @if( $count_shedules_lenders==0 && $count_shedules_branch!=0)
                  <div class="alert alert-danger" role="alert">
                     No tienes configurado ningún dia y horario para este prestador, los turnos estan tomando la configuración de la sucursal. <a href="<?php echo url('sucursal/horarios/'.$get_branch->suc_id);?>">Ver configuración</a>
                  </div>
                  @endif

                  @if( $count_shedules_lenders==0 && $count_shedules_branch==0)
                  <div class="alert alert-danger" role="alert">
                     No tienes configurado ningún dia y horario para este prestador, los turnos estan tomando la configuración de la empresa. <a href="<?php echo url('empresa/horarios/'.$get_business->em_id);?>">Ver configuración</a>
                  </div>
                  @endif
                  ...
                  <div class="form-group bdashed pptop" style="width: 100%; clear: both;">
                     ...
                     @for($i=0;$i<7;$i++)
                     <?php 
                        $shedules=DB::table('tu_dlab')->where('lab_dian',$i)->where('lab_presid',$get_lender->tmsp_id)->first();
