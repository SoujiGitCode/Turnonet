   ...
   <div class="row">
      <div class="col-md-8 col-sm-8 col-xs-12">
         ...
         <ul class="nav nav-pills">
            <li class="active"><a href="<?php echo url('agenda/empresa/'.$get_business->em_id);?>">Solicitados</a></li>
            <li><a href="<?php echo url('agenda/empresa/cancelados/'.$get_business->em_id);?>">Cancelados</a></li>
            ...
            <div class="panel">
               ...
               <div class="panel-body">
                  <div class="row">
                     ...
                     <div class="bg-list" ng-show="filteredItems > 0" style="width: 100%">
                        <div class="row hidden-xs">
                           <div class="col-md-12" style="margin-bottom: 2vw;display: none;" id="baja-selec">
                              <span class="controls" ng-show="filteredItems > 0">
                                <a onclick="openModalCancel()" class="deleteuser red"><i class="ti-trash"></i> CANCELAR TURNOS SELECCIONADOS</a>
                             </span>
                              <br>
                          </div>
                        </div>
                        <div class="row list-hives" ng-repeat="row in filtered = (list | filter:search | orderBy : predicate :reverse) | startFrom:(currentPage-1)*entryLimit | limitTo:entryLimit" >
                           <div class="col-xs-10 p04">
                              ...
                              <img src="<?php echo url('img/video-player.png');?>" title="Abrir sala" class="zoom-i hidden-xs" ng-show="row.id_meeting!=''" ng-click="goTourlZoom(row.url_zoom)" style="cursor: pointer;">



                              <h3 class="card-title" >
                               <input type="checkbox" id="item_@{{row.id}}" value="@{{row.id}}" onchange="set_items(this.id)" class="hidden-xs" title="Seleccionar el turno">   @{{row.date}} @{{row.hour}} @{{row.time}}
                              </h3>
...
<input type='hidden' id='week_start' value="<?php echo date('d-m-Y',strtotime($week_start));?>" />
<input type='hidden' id='week_end' value="<?php echo date('d-m-Y',strtotime($week_end));?>" />
