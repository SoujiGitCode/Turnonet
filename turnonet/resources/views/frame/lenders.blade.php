...
<div class="container container-single ptop" ng-app="myApp" ng-controller="post">
   ...
   @endif
   <?php $speciality=DB::table('tu_emps_serv')->select('serv_id','serv_nom')->where('serv_empid',$get_business->em_id)->where('serv_tipo','2')->where('serv_estado','1')->groupBy('serv_nom')->get(); ?>
   @if(count($speciality)!=0 || count($branch)>1)
   <div class="row" style="clear: both; @if($frame->searchbar=='1') display: none; @endif">
      <div class="col-xs-12">
         <div class="xsletter"><br>Búsqueda: </div>
         <div class="row row-search">
            @if(count($branch)>1 && count($speciality)==0)
            ...
            @if(count($speciality)!=0 && count($branch)<=1 )
            <div  class="col-md-8 col-sm-8 col-xs-12">
               <select name="espid" id="espid" class="f_sel form-control">
                  <option value="ALL">Especialidad</option>
                  @foreach($speciality as $rs)
                  <option value="<? echo $rs->serv_id; ?>"><? echo $rs->serv_nom;; ?></option>
                  @endforeach
               </select>
            </div>
            @endif
      ...
      <div class="col-md-12 bg-list"  style="display: none;" >
         <div class="row">
            <div class="col-md-12" style="text-align: center;" ng-if="filteredItems <= 0">
               ...
               <p>No hay prestadores disponibles en este momento</p>
            </div>
         </div>
