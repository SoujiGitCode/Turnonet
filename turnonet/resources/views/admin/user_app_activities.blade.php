...
<div class="content-area">
   <div class="breadcrumb-bar">
      <div class="page-title">
         <h1><i class="ti-user"></i> Registro de Actividades</h1>
      </div>
         ...
         <div class="masonary">
            <div class="col s12">
               ...
               <div class="widget-bar">
                <div class="col s12">
                  <p>Actividad reciente de <?php echo $user->us_nom;?></p>
                </div>
                  <div class="col s6">
...
<script type="text/javascript">
    ...
    app.controller('paginado', function($scope, $http, $window) {
        ...
        $scope.rows = function() {
          $http.get("{{url('/')}}/activities_listing/<?php echo $user->us_id;?>/0/0").then(function successCallback(response)  {
        ...
        $scope.search_items = function() {
           $("#op-excel").show();
            ...
            $("#btn-excel").attr("href", "{{url('/')}}/excel/activities/" + tech + "/" + start + '/' + end);
            $http.get("{{url('/')}}/activities_listing/" + tech + "/" + start + '/' + end).then(function successCallback(response)  {
                $scope.list = response.data;
                $scope.currentPage = 1;
                $scope.filteredItems = $scope.list.length;
                $scope.totalItems = $scope.list.length;
