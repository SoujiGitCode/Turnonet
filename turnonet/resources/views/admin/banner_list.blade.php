                         ...
                         <thead>
                           ...
                           <tr ng-repeat="row in filtered = (list | filter:search | orderBy : predicate :reverse) | startFrom:(currentPage-1)*entryLimit | limitTo:entryLimit">
                              ...
                              <span class="controls">
                                ...
                                <a href="{{url('/')}}/banners/@{{row.id}}/edit" class="edituser blue"><i class="ti-pencil-alt"></i> Editar</a>
...
app.controller('paginado', function($scope, $http, $window) {
    //Listar Paginas 
    $scope.rows = function() {
        $http.get("{{url('banners_lists')}}").then(function successCallback(response)  {
