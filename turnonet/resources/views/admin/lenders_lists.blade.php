...
<div class="content-area">
   <div class="breadcrumb-bar">
      <div class="page-title">
         <h1><i class="ti-agenda"></i> Prestadores</h1>
      </div>
            ...
            <div class="col s12">
               <div class="widget z-depth-1">
                           ...
                           <thead>
                              <tr>
                                ...
                                <th style="width: 20%;cursor: pointer;" title="Ordenar" ng-click="sortBy('business')">Empresa <span  class="sortorder" ng-show="propertyName === 'business'" title="Ordernar" ng-class="{reverse: reverse}"></span></th>
                                 ...
                                 <th style="width: 10%;cursor: pointer;" title="Ordenar" ng-click="sortBy('status')">Estatus <span  class="sortorder" ng-show="propertyName === 'status'" title="Ordernar" ng-class="{reverse: reverse}"></span></th>
                                 ...
                                 <td style="vertical-align: middle;">
                                  ...
                                  <select name="select_1" class="browser-default select-list" id="@{{row.id}}" onchange="selectOpt(this.id)" style="background-color: #fafafa;border:1px solid rgba(0, 0, 0, .12)">
                                   <option value="">Seleccioná</option>
                                     ...
                                     <option value="1" ng-show="row.status=='ALTA'">Dar de Baja</option>
                                     <option value="2" ng-show="row.status !='ALTA'">Dar de Alta</option>
                  ...
                  <div class="row results-table" ng-show="filteredItems != 0" style="border-top: 1px solid #ddd;">
                        ...
                        <div class="col s3">
                           <a  class="btn orange" href="<?php echo url('/');?>/excel/lenders" target="_blank" style="height: 45px;line-height: 50px; border-radius: 0.3rem;background-color: #33b56a!important;"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Descargá  Excel</a>
                        </div>
                        <div class="input-field col s12">
...
<script type="text/javascript">
   ...
   app.controller('paginado', function($scope, $http, $window) {
    ...
    $scope.sortBy = function(propertyName) {
      ...
      $scope.rows = function() {
        $("#mask").hide();
         $http.get("{{url('lenders_lists')}}").then(function successCallback(response)  {
      ...
      $scope.bajar = function(id) {
         swal({
            title: "Confirmá que querés dar de baja a este prestador",
            text: "",
            type: "warning",
         ...
         function() {
            var route = "{{url('up-status-lender')}}";
            var token = $("#token").val();
            $.ajax({
               url: route,
               headers: {
               ...
               success: function() {
                  $.growl.error({ title: "<i class='fa fa-exclamation-circle'></i> Atención", message: "El estatus del prestador ha sido actualizado"});
                  $scope.rows();
                  $('select').val('');
               },
                error: function(msj) {
      ...
      $scope.activar = function(id) {
         swal({
            title: "Confirmá que querés dar de alta a este prestador",
            text: "",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: '#CE0505',
         ...
         function() {
            ...
            $.ajax({
               ...
               success: function() {
                  $.growl.warning({ title: "<i class='fa fa-exclamation-circle'></i> Atención", message: "El estatus del prestador ha sido actualizado"});
                  $('select').val('');
                  $scope.rows();
               },
                error: function(msj) {
