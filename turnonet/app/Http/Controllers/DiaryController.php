        ...
        try {
            ...
            foreach ($shift as $rs) {
                ...
                if ($status == "BAJA") {
                    ...
                    $count_shedules = DB::table('blocked_schedules')->where('tur_id', $rs->tu_id)->count();
                    if (isset($motivo) != 0):
                ...
                if (isset($lender) != 0 && isset($user) != 0) {
                    ...
                    if ($type_shift != '3') {
                        $json[] = array(
                            "id" => $rs->tu_id,
                            "tu_st" => $rs->tu_st,
                            "id_meeting"=>$rs->id_meeting,
                            "url_zoom"=>$rs->url_zoom,
...
# code...
    ...
    public function delete_shedules_cancel(Request $request) {
        try {
            BlockedShedules::where('tur_id', $request['id'])->delete();
            return response()->json(["msg" => "borrado"]);
        } catch (Exception $ex) {
            return false;
        }
