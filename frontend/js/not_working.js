...
app.controller('posts', function($scope, $http, $window) {
    $scope.titulo = "Registrá los días no laborables de la empresa aquí:";
    $scope.btn = "Agregar";
    ...
    $scope.rows = function() {
        ...
        $(".bg-preload").show();
        $(".bg-list").hide();
        $http.get($("#url").val() + '/lists_holidays/' + $("#business").val()+'/' + $("#type").val()).then(function successCallback(response) {
   ...
   $scope.filter = function() {
    ...
    $scope.asignarValor = function(id, name, desde) {
        ...
        $("#hasta").hide();
        $scope.titulo = "Actualizá el día no laborable de la empresa aquí:";
      ...
      $scope.borrar_all = function(id) {
        swal("Confirmá que querés dar de baja la selección", {
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        url: $("#url").val() + '/up_status_working_business',
                        type: 'POST',
                        data:{ id: $scope.items_selected, status:'2',business: $("#business").val() },
                        dataType: 'json',
                        success: function() {
                            $.growl.error({
                                title: "<i class='fa fa-exclamation-circle'></i> Atención",
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        url: $("#url").val() + '/up_status_working_business',
                        type: 'POST',
                        data:{ id: $scope.items_selected, status:'1',business: $("#business").val() },
                        dataType: 'json',
                        success: function() {
                            $.growl.error({
                                title: "<i class='fa fa-exclamation-circle'></i> Atención",
