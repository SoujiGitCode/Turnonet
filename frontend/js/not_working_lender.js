...
app.controller('posts', function($scope, $http, $window) {
    $scope.titulo = "Registrá los días no laborables del prestador aquí:";
    $scope.btn = "Agregar";
    ...
    $scope.rows = function() {
        ...
        $(".bg-preload").show();
        $(".bg-list").hide();
        $http.get($("#url").val() + '/lists_holidays_lender/' + $("#select_lender").val()+'/' + $("#type").val()).then(function successCallback(response) {
    ...
    $scope.asignarValor = function(id, name, desde) {
        ...
        $("#hasta").hide();
        $scope.titulo = "Actualizá el día no laborable del prestador aquí:";
    ...
    $scope.optCan = function(date) {
        swal("Confirmá que querés dar de baja el día no laborable", {
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        url: $("#url").val() + '/delete_holiday_lender',
                        type: 'POST',
                        dataType: 'json',
                        data: {
                            date: date,
                            lender: $("#select_lender").val()
                        },
                        success: function() {
                            $.growl.error({
    ...
    $scope.borrar_all = function(id) {
        swal("Confirmá que querés dar de baja la selección", {
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        url: $("#url").val() + '/up_status_working_lender',
                        type: 'POST',
                        data:{ id: $scope.items_selected, status:'2',lender: $("#select_lender").val() },
                        dataType: 'json',
                        success: function() {
                            $.growl.error({
                                title: "<i class='fa fa-exclamation-circle'></i> Atención",
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        url: $("#url").val() + '/up_status_working_lender',
                        type: 'POST',
                        data:{ id: $scope.items_selected, status:'1',lender: $("#select_lender").val() },
                        dataType: 'json',
                        success: function() {
                            $.growl.error({
                                title: "<i class='fa fa-exclamation-circle'></i> Atención",
