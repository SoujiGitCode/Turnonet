...
function goToEmp(url) {
    if (p_utr == 0) {
        p_utr = 1;
        window.location = $("#url").val() + '/' + url + '/' + $("#branch").val();
        return false;
    }
}
$('#branch').on('change', function() {
    ...
    if (pathName != '/sucursales') {
        ...
        window.location = $("#url").val() + '/sucursal/editar/' + $("#branch").val();
    }
});
...
function update_notifications() {
    ...
    var route = $("#url").val() + '/update_notifications_branch';
    $.ajax({
        url: route,
        type: 'POST',
        dataType: 'json',
...
function baja_branch() {
    swal("Confirmá que querés dar de baja a la sucursal", {
            buttons: {
                cancel: "No",
                confirm: "Si"
            },
        ...
        .then((willDelete) => {
            if (willDelete) {
                $("#boton-1").prop('disabled', true);
                $("#boton-1").html('<i class="fa fa-circle-o-notch fa-spin"></i> Procesando');
                var route = $("#url").val() + '/update_status_branch';
                $.ajax({
                    url: route,
                    type: 'POST',
                    dataType: 'json',
                    ...
                    success: function(data) {
                        if (data.msg != "error") {
                            ...
                            swal("Su sucursal ha sido dado de baja", {
                                    allowOutsideClick: false,
                                    closeOnClickOutside: false
                                })
                                .then((value) => {
...
function update_settings() {
    if ($("#hours").val() == "00" && $("#minutes").val() == "00") {
        ...
        swal("Confirmá que querés actualizar la configuración de la sucursal", {
                buttons: {
                    cancel: "No",
                    confirm: "Si"
                },
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $("#boton-1").prop('disabled', true);
                    $("#boton-1").html('<i class="fa fa-circle-o-notch fa-spin"></i> Procesando');
                    var route = $("#url").val() + '/update_settings_branch';
                    $.ajax({
                        url: route,
                        type: 'POST',
                        dataType: 'json',
...
function update_shedules() {
    ...
    /*Saturday*/
        ...
        $("#boton-1").prop('disabled', true);
        $("#boton-1").html('<i class="fa fa-circle-o-notch fa-spin"></i> Procesando');
        var route = $("#url").val() + '/update_shedules_branch';
        $.ajax({
            url: route,
            type: 'POST',
            dataType: 'json',
...
function create_lender() {
    ...
    if ($("#tmsp_pnom").val() == "") {
        ...
        $("#tmsp_pmail").focus();
        return false;
    } else if (!expr.test($("#tmsp_pmail").val())) {
        ...
        $.ajax({
            ...
            if (obj.msg != "error") {
                $("#boton-1").prop('disabled', false);
                $("#boton-1").html('Guardar');
                swal("Su prestador ha sido registrado con éxito", {
                        allowOutsideClick: false,
                        closeOnClickOutside: false
                    })
                    .then((value) => {
                        window.location = $("#url").val() + "/sucursal/prestadores/" + $("#select_branch").val();
                    });
            }
        }).fail(function(jqXHR, textStatus) {
            $("#boton-1").prop('disabled', false);
...
function send_reports() {
    if ($("#day").val() == "") {
        ...
        a = new FormData(document.getElementById("form"));
        $.ajax({
            url: $("#url").val() + '/send_reports_branch',
            type: "post",
            dataType: "html",
            data: a,
            cache: !1,
...
function download_report() {
    ...
    if ($("#day").val() == "") {
        ...
        $.ajax({
            url: $("#url").val() + '/verify_reports_branch',
            type: 'POST',
            dataType: 'json',
            data: $("#form").serialize(),
            success: function(data) {
                if (data.msg != "error") {
                    var type = $('input:radio[name=rep_type]:checked').val();
                     window.open($("#url").val() + "/pdf/reporte/2/" + type + "/" + $("#suc_id").val() + "/" + $("#emp_id").val() + "/" + $("#day").val());
                } else {
                    launch_toast('No hay turnos solicitados para esta fecha');
